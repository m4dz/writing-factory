## call 1 — write.opening — model=fake num_predict=300 temperature=0.7
...
Plan du chapitre (>> = l'entrée à écrire maintenant) :
  1. >> Chapitre 2, entrée unique du carnet, 450-600 mots.
L'en-tête et la citation sont DÉJÀ ÉCRITS.
Ce qui reste, dans l'ordre : elle relit, constate l'écart, va vérifier ; elle rétablit sa soirée ; elle rend son verdict ; le corps constate, une ligne ; une phrase brève referme.
Le soir : elle relit l'entrée de la veille de son cahier, comme chaque soir. Sa mémoire dit une assiette, un dîner seule ; l'entrée en compte deux. Elle va voir : la cuisine, l'égouttoir — regarder, compter. Ce qu'elle voit confirme l'entrée, pas sa mémoire. Elle se donne une raison : la fatigue, l'automatisme.
Verdict à rendre, dans ses mots : erreur de relevé — la faute est à elle, pas au texte ; et la résolution de pointer plus précisément.
Dans la maison, ce soir : le cahier, l'assiette, l'égouttoir.
CE QU'ELLE VOIT, imposé : « L'égouttoir, ce soir : deux assiettes. » Elle le CONSTATE, elle ne le découvre pas.
...
Ce que l'entrée 1 raconte : Chapitre 2, entrée unique du carnet, 450-600 mots.
L'en-tête et la citation sont DÉJÀ ÉCRITS.
Ce qui reste, dans l'ordre : elle relit, constate l'écart, va vérifier ; elle rétablit sa soirée ; elle rend son verdict ; le corps constate, une ligne ; une phrase brève referme.
Le soir : elle relit l'entrée de la veille de son cahier, comme chaque soir. Sa mémoire dit une assiette, un dîner seule ; l'entrée en compte deux. Elle va voir : la cuisine, l'égouttoir — regarder, compter. Ce qu'elle voit confirme l'entrée, pas sa mémoire. Elle se donne une raison : la fatigue, l'automatisme.
Verdict à rendre, dans ses mots : erreur de relevé — la faute est à elle, pas au texte ; et la résolution de pointer plus précisément.
...
## call 2 — write.reconstruction — model=fake num_predict=800 temperature=0.7
...
## call 3 — write.closing — model=fake num_predict=260 temperature=0.7
...
## call 4 — accumulate — model=mistral-nemo:12b-instruct-2407-q8_0 num_predict=300 temperature=0.3
...
## call 5 — review — model=fake num_predict=1200 temperature=0.5
...
## call 6 — repair — model=qwen2.5:7b-instruct num_predict=788 temperature=0.2
...
## call 7 — qa.facts — model=qwen2.5:7b-instruct num_predict=400 temperature=0.1
...
## call 8 — qa.questions — model=qwen2.5:7b-instruct num_predict=500 temperature=0.1
...
## call 9 — qa.answers — model=qwen2.5:7b-instruct num_predict=400 temperature=0.0
