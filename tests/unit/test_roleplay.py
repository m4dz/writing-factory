...
def test_out_of_role_markers_both_ways():
    assert roleplay.hors_role(fx.OUT_OF_ROLE) == ["programme informatique",
                                                  "simuler des conversations"]
    assert roleplay.hors_role("Je corrige des manuscrits, laissez-moi.") == []
    assert roleplay.hors_role("En tant qu'IA, je ne peux pas.")


def test_reply_cleaning_removes_double_dash_and_own_name():
    assert roleplay._nettoyer("— — Non, je ne peux pas.") == "— Non, je ne peux pas."
    assert roleplay._nettoyer("Judith : Ah, ma chère…", "Judith") == "Ah, ma chère…"
...
def test_session_turn_and_persistent_out_of_role_is_excluded_from_memory(
    ...
    assert s.transcription[-1]["hors_role"] is True
...
def test_close_writes_indexes_and_replays(fake_model, fake_chroma, sessions_dir):
    ...
    listed = roleplay.lister_sessions("judith")
    assert listed and listed[0]["character"] == "judith"
    replay = roleplay.lire_session("judith", listed[0]["horodatage"])
    ...
    assert s2.rappels and "CE DONT TU TE SOUVIENS" in s2._system()
