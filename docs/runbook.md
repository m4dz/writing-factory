...
A consumed swap blocks only in timing mode (`run_chapter.py`, the API), where
...
```bash
orchestrator/.venv/bin/python outillage/etancheite.py > experiments/reports/rapport-etancheite-$(date +%Y%m%d).md
```

### 1.4 Orchestrator venv
...
```bash
cd orchestrator
.venv/bin/python -u run_chapter.py --brief "Une entrée du carnet…" --characters judith
```

Chapter 7 is NOT generated by this generic CLI: its structure lives in
`orchestrator/ch7.py` and is triggered by the API or by
...
```bash
cd orchestrator
.venv/bin/python api.py            # listens on 0.0.0.0:8420
```
...
| H-20 | `caffeinate -is &`, then `preflight.py`: all green |
